      ...
      <div class="container">
        <div class="row d-flex align-items-center justify-content-center">
          <div class="banner-content col-lg-12 col-md-12">
            <h1 class="label-name">{{ current_category }}</h1>
          </div>
        </div>
      </div>
    </section>
...
export default {
  ...
  methods: {
    ...
    async navigateToCategory(category) {
      try {
        ...
        await this.getAllEventsByCategory(id);
  ...
  async created() {
    ...
    this.current_category = category.split("+").join(" ");
    this.setHistory(category);

    if (this.allCategories.length === 0) {
      this.fetchAllCategories();
    }
    const id = this.findCategoryId(this.current_category);
    await this.getAllEventsByCategory(id);
    this.ready_to_display = true;
  },
};
