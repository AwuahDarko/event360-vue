<template>
    <div class="hold-transition register-page">
        ...
        <div class="card">
            ...
            <h1></h1>
            <p class="login-box-msg">Register with event360</p>
            ...
            <form @submit="signUp">
                ...
                <div class="input-group mb-3">
                <input type="password" class="form-control"  v-model="conPassword" placeholder="Retype password" required>
                <div class="input-group-append">
                    <div class="input-group-text">
                    <span class="fas fa-lock"></span>
                    </div>
                ...
                <div class="col-8">
                    ...
                    <input type="checkbox" id="agreeTerms" name="terms" v-model="agreed">
        ...
        <div class="row my_row">
            <div class="col-md-4 col-sm-4 col-xm-6" >
                    <a href="https://event360-gh.com/auth/facebook" class="fb connect">Facebook</a>
                </div>
            ...
            <p>Already have an event360 account? <router-link to="login" class="text-center">Login in.</router-link></p>
            </div>
            <!-- /.form-box -->
        </div><!-- /.card -->
        </div>
    ...
    methods: {
        validatePassword(){
            if(this.password !== this.conPassword){
                this.$refs.snackbar.error('Your passwords do not match')
                return false;
            }

            if(this.password.length < 8){
                this.$refs.snackbar.error('Your passwords must be at least 8 characters long')
                return false;
            }

            if(!this.agreed){
                this.$refs.snackbar.error('Please read our terms and agree to proceed')
                return false;
            }
        ...
        signUp(evt){
            ...
            this.$Progress.start()
            const body = JSON.stringify(this.setBody())
            const options = {
                method: 'POST',
                headers: {
            ...
            fetch(`${apiUrl}/auth/local/sign-up`, options)
            .then(res => {
                this.$Progress.finish(); 
                if(res.status === 201){
                    window.localStorage.setItem('return_to_login_info', 'Sign up was successful, please login now')
                    this.$router.push('login')
                }else{
                    return res.text()
                }
            })
            .then(text => this.$refs.snackbar.error(text))
            .catch(err => { console.log(err) ; this.$Progress.finish(); })
        }
    }
}
</script>
